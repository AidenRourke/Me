...
**Concepts:** Payment systems (Stripe, Zuora), 2FA, performance optimization, data visualization
...
- Drove core backend work in migrating the company's billing system from Zuora to Stripe: untangled Zuora dependencies woven throughout the application and reimplemented its billing behaviour against the Stripe API in a new subscription-management microservice.
- Took an MCP tool over the company's query infrastructure from hackday concept to full technical spike, connecting it to the product's AI chatbot to turn natural-language questions into queryable metrics and data feeds.
...
- Designed and built an internal tool for the support team to summarize partner-account limits and usage across clients, aggregating data spread across multiple microservices via an async loading job; proposed the design and secured stakeholder approval.
- Integrated JDBC-compatible data warehouses (BigQuery, Snowflake, PostgreSQL) into the connection service, handling varied drivers, query languages, and authentication so users could define metrics in their own warehouses.
...
- Built and deployed a personal calendar server (Node.js/Express) on an old Mac via pm2, served privately over Tailscale.
